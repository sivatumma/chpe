...
(function(chUtils) {
  ...
  chUtils.setDefaultDate = function(schemeData, orderData) {
    ...
    if (schemeData[0].metadata.type == "COUPON") {
      var datediff =  parseInt((schemeData[0].behavior.endDate.toISOString("DD/MM/YYYY") - schemeData[0].behavior.startDate.toISOString("DD/MM/YYYY")) / (1000 * 60 * 60 * 24)); 
      //var datediff = schemeData[0].behavior.endDate.getDate() - schemeData[0].behavior.startDate.getDate();
      ...
      if (datediff <=7) {
        ...
        console.log(revenues_and_discounts.discounts);
  }
  else
  {
