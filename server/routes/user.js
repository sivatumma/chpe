...
module.exports = function (app){
    ...
    app.use(compression({threshold:0}));
       ...
       if(!req.body.username || !req.body.password) return res.status(500).send({status:"fail",message:'Invalid request params.'});
        User.getAuthenticated(req.body.username,req.body.password,function (err,doc,reason){
            ...
            var token_object={token:uuid.v4(),token_created:new Date(),token_expires:(new Date().addMinutes(15))};
    ...
    app.post('/register',function (req,res){
        if(!req.body.username || !req.body.password || !req.body.age || !req.body.gender) return res.send(500,{message:'Invalid request params.',status:500});
