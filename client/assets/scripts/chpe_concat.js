...
function generalConfig() {
	var urls = {
		...
		memcache: {
			...
			serviceLevelDiscounts: '/api/proxy?url=http://52.76.37.144:8080/javaapi/rest/serviceLevelDiscounts/',
			serviceRateCategories: '/api/proxy?url=http://52.76.37.144:8080/javaapi/rest/serviceRateCategories/',
...
(function(chUtils) {
  ...
  chUtils.setDefaultDate = function(schemeData, orderData) {
    ...
    if (schemeData[0].metadata.type === "COUPON") {
      ...
      if (datediff <= 7) {
        for (var i = 1; i <= 7; i++) {
          ...
          unique_data[i + " Day " + monthNames[month - 1]] = 0;
        }
        _.each(groupedByMonth, function(item, key) {
          ...
          unique_data[parseInt(dYearDate[1]) + " Day " + monthNames[dYearDate[0] - 1]] = uniqueList.length;
        ...
        _.each(groupedByMonth, function(item, key) {
          ...
          unique_data[week + monthNames[dYearDate[0] - 1]] = uniqueList.length;
      ...
      for (var i = 0; i < 13; i++) {
        if (month <= 12) {
          ...
          unique_data[monthNames[month - 1] + " " + year] = 0;
          month++;
        } else {
          month = 1;
          year++;
      ...
      _.each(groupedByMonth, function(item, key) {
        ...
        unique_data[monthNames[dYearDate[1] - 1] + " " + dYearDate[0]] = uniqueList.length;
      });
    ...
    orderDetails.unique_customers = unique_data;
    finalData = {
